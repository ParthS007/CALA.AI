  ...
  try {
    ...
    // Handle error
      ...
      async function sendMessageToServer(message) {
        if (message.startsWith("Give Kudos")) {
          const name = message.slice("Give Kudos".length).trim();
          if (name) {
            await fetch('/record-kudos', {
              method: 'POST',
              headers: { 'Content-Type': 'application/json' },
              body: JSON.stringify({ employeeName: name })
            });
          }
          return; // Skip the rest of the function to avoid calling the OpenAI API
        }
        let imageUrl = null;
