...
// Serve uploaded files from the 'frontend/uploads' directory
app.get('/uploads/:filename', (req, res) => {
  const filename = req.params.filename;
  res.sendFile(filename, { root: 'frontend/uploads' });
});
...
app.post('/record-kudos', (req, res) => {
  const { employeeName } = req.body;
  const csvPath = './employee_data.csv';
  ...
  for (const record of records) {
    if (record.name === employeeName) {
      record.kudos = parseInt(record.kudos || '0', 10) + 1;
      found = true;
      break;
    }
  }
  ...
  if (!found) {
    records.push({ name: employeeName, kudos: 1 });
  }

  const output = stringify(records, { header: true });
  fs.writeFileSync(csvPath, output);
  
  res.json({ message: `Kudos recorded for ${employeeName}` });
});
